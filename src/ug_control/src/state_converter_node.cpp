/**
 * @file state_converter_node.cpp
 * @brief 状态转换节点：Gazebo ground truth (ENU) → 滑翔机状态 (NED)
 *
 * 订阅：/ground_truth/pose (nav_msgs/Odometry, Gazebo P3D 输出)
 * 发布：/{ns}/glider_state (ug_msgs/GliderState, NED 坐标系)
 *
 * 坐标转换：
 *   NED.north = ENU.x     (ENU X = 东? 不对，Gazebo ENU: X=东 Y=北 Z=上)
 *   实际上 Gazebo 默认不是 ENU，而是 X=前, Y=左, Z=上。
 *   UUV simulator 在 NED 辅助节点中处理了这个，
 *   这里我们直接读 Gazebo 世界坐标并做映射：
 *     Gazebo world: X=North, Y=East (假设初始朝北), Z=Up
 *     NED: X=North, Y=East, Z=Down
 *   所以：north=X, east=Y, depth=-Z
...
public:
  ...
  void onPose(const nav_msgs::Odometry::ConstPtr &msg)
  {
    ...
    // 位置：Gazebo (X-North, Y-East, Z-Up) → NED (North, East, Down)
