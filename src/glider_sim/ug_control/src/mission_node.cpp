 ...
 * 导航状态机
 *
 *   自设置模式IDLE， 通过dynamic_reconfigure设定target_depth直接控制深度
 *   ZIGZAG模式，接收目标航点，做锯齿运动过去
 *
 * 状态机: IDLE；DIVING,CLIMBING, ARRIVED, 循环
 *
 * 订阅话题，ns依旧是ug_glider
 *   /{ns}/glider_state          (GliderState)  当前状态
 *   /{ns}/mission/target        (PoseStamped)  目标点 (world frame, x=north, y=east)
 *   /move_base_simple/goal      (PoseStamped)  RViz 2D Nav Goal
 *
 * 发布话题，
 *   /{ns}/cmd/depth             (Float64)      深度指令
 *   /{ns}/cmd/heading           (Float64)      航向指令
 *   /{ns}/mission/state         (MissionState) 任务状态 (给viz用)
 */
...
private:
  ...
  {
    // 2D Nav Goal: 同样 world frame x,y
    targetNorth_ = msg->pose.position.x;
    targetEast_ = msg->pose.position.y;
    hasTarget_ = true;
    startMission();
    ROS_INFO("[Mission] 2D Nav Goal 接收: north=%.1f, east=%.1f",
  ...
  // ROS
  ros::NodeHandle nh_;
  ros::Subscriber stateSub_, targetSub_, navGoalSub_;
